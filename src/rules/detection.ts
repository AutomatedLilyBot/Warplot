...
import { type Ctx, classOf, entityExists, entityInfo, entityPosition, isEmitting, sensorDef } from './world.js';
...
export function detectability(
  ...
  const targetCategory = s.units[target] ? classOf(ctx, s.units[target]).category : 'missile';
  const suitableTarget = !sd.targetCategories || sd.targetCategories.includes(targetCategory);
  ...
  const checks = [
    check(`${sd.name} 开机`, on),
    check('目标存在', exists),
    check(`目标类别 ${targetCategory} 适用`, suitableTarget),
  ...
  const ok = on && exists && suitableTarget && inRange && !blocker && emitting;
