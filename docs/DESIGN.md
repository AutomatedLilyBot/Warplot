...
- `SensorDef`（现状）：`rangeM`、`maxQuality`、`emits`、`requiresTargetEmission`（ESM）、`reofferIntervalS`、`targetCategories`（目标类别白名单，可选）
...
- 区分探测、分类、持续跟踪与武器支持门限；禁止仅凭一次“距离够近”直接获得最高质量航迹。场景数据缺少雷达方程参数时，应标记为旧模型/待迁移，不能把 `rangeM × signature` 伪装成新规则。
...
**孔径、传播与散射。** 快子束的自由空间通量按 $1/(4\pi R^2)$ 扩散，允许相干阵列。理想圆形孔径的衍射尺度 $\theta_{\rm diff}\approx1.22\lambda/D$；稀疏阵列的实际波束形状和旁瓣另由阵列定义。阵列基线 $D$ 决定角尺度，实际发射/接收有效面积或填充效率另决定增益，稀疏长基线不得按填满的圆盘计算增益。目标使用独立于电磁 RCS 的快子等效散射截面 $\sigma_t$，第一版为标量；遮挡和额外损失由环境配置。单站、远场、静止目标的约定工程方程为
...
其中 $Q_1$ 是一阶 Marcum Q 函数，$P_{\rm FA}$ 是**单个搜索单元**的虚警率。多单元搜索还须计算总虚警风险；有线索指向时，整次更新概率须对先验目标位置积分 $P_{D,\rm total}=\int P_D(x)\,p_{\rm prior}(x)\,dx$，计入波束边缘增益与未照射区域。固定发射功率、TCS、孔径和噪声，且 $v_g\gg c$ 时 $\lambda\approx\lambda_\infty$，$P_D$ 不因单个快子能量趋零而无限增大。未来可选的计数接收器必须另给能量相关响应 $\eta(E)$，不得假定其在 $E\to0$ 时恒定。
...
**输入与输出。** 世界规则保存 $\mu$、优选参考系、遮挡与默认噪声/判定约定；设备保存 $E_0$ 或 $\beta_t$、能谱/编码波形、平均发射功率、发射和接收孔径及有效面积、接收噪声、驻留时间、扫描区域、单元虚警率、指向/时钟/波形标定和色散补偿精度；目标保存 $\sigma_t$，环境保存损失与遮挡。引擎输出传播时刻、回波功率、SNR、$P_D$、测角/测距误差及带观测时刻的三维测量协方差，再交给统一航迹滤波器。不能用单个离散 `TrackQuality` 取代概率区域；分类可信度与具体武器能否使用该航迹由结果另行推导。

**银河尺度校准与验收。** 以有效孔径/基线 $D=1.39\times10^9$ m、$R=100{,}000$ ly、$v_g=10^6c$、$\lambda_\infty=1$ cm、太阳尺度目标及 10 分钟有效驻留为基准。由此 $\mu\approx2.2102\times10^{-40}$ kg，$\nu\approx29.98$ kHz，衍射尺度约 $0.0555$ AU；中心能量的单程/往返约 36.525/73.05 天。校准目标是目标处于有效照射范围时的单次条件探测率 $P_D\approx0.95$，三维 **95% 联合概率椭球**横向半轴约 $0.05$ AU、径向半轴约 $0.01$ AU。径向目标对应 $\sigma_{\rm time}\approx3.57$ μs；在 $P_{\rm FA}=10^{-6}$ 的上述未知相位判定约定下，探测目标对应 $\rho\approx23.24$。这些是校准目标及反算约束，不是设备硬编码值；单个基准点不能唯一拆出发射功率、TCS、噪声与各项误差预算。
...
实施前须验证 1 万、10 万和 100 万光年，$10^4c$、$10^6c$ 和 $10^8c$，更小目标 TCS，以及连续观测后的航迹收敛。该孔径与 1 cm 波长的远场起点约为 40,800 ly，所以 1 万光年测试必须使用近场模型或标记远场公式无效。验证还须包括先验指向覆盖、脉冲/编码歧义、宽能谱回波的接收窗口、系统误差下限及目标机动外推。数十天的往返时间不等于同样天数的观测时龄：中心频率回波在收到时约对应 **36.525 天前**的目标状态。暂不实施完整量子场论、洛伦兹系间的超光速因果推演、快子偏振、介质折射、多径和完整时域波形传播。
